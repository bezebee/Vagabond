...
const initiallyDisplayedCurrencies = ["AUD", "EUR", "GBP", "JPY", "RUB"];
let baseCurrency;
...
//coding the span button to remove a currency from the main list
...
function setNewBaseCurrency(newBaseCurrencyLI) {
  ...
  currenciesList.querySelectorAll(".currency").forEach((currencyLI) => {
    ...
    currencyLI.querySelector(
      ".base-currency-rate"
    ).textContent = `1 ${baseCurrency} = ${exchangeRate} ${currencyLI.id}`; // set the text content for the base currency element
  });
}
...
const isNewBaseCurrency = event.target.closest("li").id!==baseCurrency;
  ...
  if (baseCurrencyAmount!== newBaseCurrencyAmount || isNewBaseCurrency) {
    ...
    const exchangeRate = currencyLI.id === baseCurrency ? 1 : (currencyRate / baseCurrencyRate).toFixed(4); //calculate currency rate
...
function populateAddCurrencyList() {
  for (let i = 0; i < currencies.length; i++) {
    addCurrencyList.insertAdjacentHTML(
      "beforeend",
      `<li class="currency list-group-item" data-currency=${currencies[i].abbreviation}>
        <img src=${currencies[i].flagURL} class="flag"/><span>${currencies[i].abbreviation}<br>${currencies[i].name}</span>
              </li>`
    );
  }
}
...
function newCurrenciesListItem(currency) {
  ...
  currenciesList.insertAdjacentHTML(
    ...
    <img src=${
      currency.flagURL
    } class="flag"><input class="curr-place" placeholder="0.0000" value=${inputValue} >
    <div class="info">
    <p class="input flag"></p>
